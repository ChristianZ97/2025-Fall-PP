...
for csv_file in csv_files:
    ...
    try:
        ...
        if df.empty or 1 not in df['Cores'].values:
            print(f"Warning: Testcase {testcase_num} has no valid data or 1-core baseline")
            continue
        ...
        print(f"Processed TC{testcase_num}: N={N:,}, max speedup={df['Speedup'].max():.2f}x")
...
# 總標題
fig.suptitle('Strong Scaling: Speedup Comparison (Stratified by Problem Size)', 
...
ax.set_title('Strong Scaling: Speedup Comparison (Grouped by Problem Size)', 
...
ax.legend(loc='center left', bbox_to_anchor=(1.01, 0.5), 
          fontsize=8, ncol=1, framealpha=0.95, shadow=True)
...
for bin_idx in bins_with_data:
    ...
    if bin_data:
        ...
        print(f"Group {bin_idx+1} ({bin_label:30s}): {len(bin_data):2d} testcases | "
