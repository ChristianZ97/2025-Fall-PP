...
for csv_file in csv_files:
    ...
    try:
        ...
        df['CPU_Time'] = df.apply(
            lambda row: max(0, row['Total_Time_srun'] - row['IO_Time'] - row['Comm_Time']), 
            axis=1
        )
    ...
    ax.bar(x_pos, y_comm, bar_width, bottom=y_cpu, label='Communication Time', color=COMM_COLOR)
    ax.bar(x_pos, y_io, bar_width, bottom=y_cpu + y_comm, label='I/O Time', color=IO_COLOR)
    
    for i, (idx, row) in enumerate(df.iterrows()):
        total_time = row['Total_Time_srun']
        ax.text(x_pos[i], total_time, f'{total_time:.1f}',
                ha='center', va='bottom', fontsize=7)
    
    ax.set_title(f'Testcase {testcase_num}: Execution Time with Increasing Parallelism', 
    ...
    if 1 not in df['Cores'].values:
        print(f"Warning: No 1-core baseline found for {csv_file}. Using minimum cores as baseline.")
    ...
    ax.set_title(f'Testcase {testcase_num}: Speedup with Increasing Parallelism', 
                 fontsize=12, fontweight='bold')
    ax.set_xlabel('Number of Processes (p)', fontsize=10)
