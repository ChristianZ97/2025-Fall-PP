...
int main(int argc, char *argv[]) {
    ...
    int work_size = width * height;
    if (work_size < 100000) num_threads = (num_threads > 2) ? 2 : num_threads;
    else if (work_size < 1000000) num_threads = (num_threads > 4) ? 4 : num_threads;

    const int min_tasks = num_threads * 4;
    int chunk_size = height / min_tasks;
    ...
    for (int i = 0; i < num_threads; i++) {
        ...
        t_args[i].chunk_size = chunk_size;
...
void *local_mandelbrot(void *argv) {
    ...
    const int chunk_size = t_arg->chunk_size;
