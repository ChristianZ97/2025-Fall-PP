...
for config in "${HW2B_CONFIGS[@]}"; do
    num_nodes=$(echo $config | awk '{print $1}')
    num_procs=$(echo $config | awk '{print $2}')
    threads_per_proc=$(echo $config | awk '{print $3}')
    ...
    for testcase_file in "${TESTCASE_FILES[@]}"; do
        results=$(run_and_parse "$HW2B_EXE" "$testcase_file" "srun -N${num_nodes} -n${num_procs} -c${threads_per_proc}")
        if [[ $? -eq 0 ]]; then
