    ...
    while (1) {
        ...
        for (int j = my_start_row; j < my_end_row; ++j) {
            ...
            for (int i = 0; i <= width - 2; i += 2) {
                __m128d repeats_vec = _mm_setzero_pd();
                ...
                for (int r = 0; r < iters; ++r) {
                    __m128d x2 = _mm_mul_pd(x_vec, x_vec);
                    __m128d y2 = _mm_mul_pd(y_vec, y_vec);

                    __m128d len_sq = _mm_add_pd(x2, y2);
                    __m128d cmp = _mm_cmplt_pd(len_sq, four);

                    repeats_vec = _mm_add_pd(repeats_vec, _mm_and_pd(cmp, one));

                    const int mask = _mm_movemask_pd(cmp);
                    if (!mask) break;

                    __m128d xy = _mm_mul_pd(x_vec, y_vec);
                    y_vec = _mm_add_pd(_mm_add_pd(xy, xy), y0_vec);
                    x_vec = _mm_add_pd(_mm_sub_pd(x2, y2), x0_vec);
                }

                __m128i int_vec = _mm_cvtpd_epi32(repeats_vec);

                int *image_ptr = &image[j * width + i];
                _mm_storel_epi64((__m128i *)image_ptr, int_vec);
            }
