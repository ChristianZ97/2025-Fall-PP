...
// REMOVED: pthread_mutex_t task_mutex = PTHREAD_MUTEX_INITIALIZER;
// REMOVED: int next_row = 0;
...
typedef struct {
    ...
    int thread_id;     // ADDED: Thread ID for static interleaved allocation
    int num_threads;   // ADDED: Total number of threads
#ifdef PROFILING
    double compute_time;
    double sync_time;
#endif
...
int main(int argc, char *argv[]) {
    ...
    for (int i = 0; i < num_threads; i++) {
        ...
        t_args[i].thread_id = i;           // ADDED: Assign thread ID
        t_args[i].num_threads = num_threads;  // ADDED: Pass total threads
#ifdef PROFILING
...
void *local_mandelbrot(void *argv) {
    ...
    // ADDED: Get thread ID and total threads for static interleaved allocation
    const int thread_id = t_arg->thread_id;
    ...
    // MODIFIED: Static interleaved allocation - each thread processes rows with stride
    // Thread 0: rows 0, num_threads, 2*num_threads, ...
    // Thread 1: rows 1, num_threads+1, 2*num_threads+1, ...
    // Thread i: rows i, i+num_threads, i+2*num_threads, ...
    ...
    t_arg->sync_time = local_sync_time;  // Remains 0 for static allocation
#endif
