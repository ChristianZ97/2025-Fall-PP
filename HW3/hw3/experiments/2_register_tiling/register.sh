...
# --- Configuration ---
OUTPUT_CSV="register_perf_results.csv"
SUMMARY_CSV="register_perf_summary.csv"
TEMP_OUT_FILE="temp_register_output.bin"
...
# --- Step 1: Compilation ---
echo "Step 1: Compiling all register tiling versions..."
make clean > /dev/null
make all
if [ $? -ne 0 ]; then
...
echo "Version,RegisterTiling,Testcase,TotalTime_ms,ComputeTime_ms" > "$OUTPUT_CSV"
echo "Version,RegisterTiling,GrandTotalTime_ms,TotalComputeTime_ms" > "$SUMMARY_CSV"

# --- Step 3: Run Experiments ---
# Find executables matching hw3-2_reg_*
EXECUTABLES=($(find . -maxdepth 1 -type f -executable -name "hw3-2_reg_*" | sort))

if [ ${#EXECUTABLES[@]} -eq 0 ]; then
    echo "Error: No executables found matching hw3-2_reg_*"
    exit 1
fi
...
for exe in "${EXECUTABLES[@]}"; do
    ...
    # Extract Strategy Name (e.g., hw3-2_reg_on -> on)
    reg_strategy=$(echo "$exe_name" | sed 's/hw3-2_reg_//')
    
    echo "=========================================="
    echo "Testing Version: $exe_name (Register Tiling: $reg_strategy)"
    echo "=========================================="
    ...
    for testcase in "${TESTCASES[@]}"; do
        ...
        if [[ -z "$prof_line" ]]; then
            ...
            echo "$exe_name,$reg_strategy,$tc_name,$total_time,$compute_time" >> "$OUTPUT_CSV"
        fi
    done
    ...
    echo "$exe_name,$reg_strategy,$grand_total_time,$grand_compute_time" >> "$SUMMARY_CSV"
