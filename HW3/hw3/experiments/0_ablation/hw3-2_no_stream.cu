...
void block_FW(
    ...
    *calls_phase3 = 0;
    ...
    for (int r = 0; r < round; ++r) {
...
#ifdef PROFILING
        cudaEventRecord(e_p1_start, 0);
#endif
        kernel_phase1<<<1, threads_per_block, 0, 0>>>(d_D, r, V_padded);
        cudaEventRecord(event_p1_done, 0);
#ifdef PROFILING
        cudaEventRecord(e_p1_stop, 0);
        ...
        // cudaStreamWaitEvent(0, event_p1_done, 0);
#ifdef PROFILING
        cudaEventRecord(e_p2_row_start, 0);
#endif
        kernel_phase2_row<<<round, threads_per_block, 0, 0>>>(d_D, r, V_padded);
        cudaEventRecord(event_p2_row_done, 0);
#ifdef PROFILING
        cudaEventRecord(e_p2_row_stop, 0);
        ...
        // cudaStreamWaitEvent(0, event_p1_done, 0);
#ifdef PROFILING
        cudaEventRecord(e_p2_col_start, 0);
#endif
        kernel_phase2_col<<<round, threads_per_block, 0, 0>>>(d_D, r, V_padded);
        cudaEventRecord(event_p2_col_done, 0);
#ifdef PROFILING
        cudaEventRecord(e_p2_col_stop, 0);
        ...
        // cudaStreamWaitEvent(0, event_p2_row_done, 0);
        // cudaStreamWaitEvent(0, event_p2_col_done, 0);

        // 3. Phase 3: Independent Blocks
        // (round, round) blocks per grid
#ifdef PROFILING
        cudaEventRecord(e_p3_start, 0);
#endif
        kernel_phase3<<<dim3(round, round), threads_per_block, 0, 0>>>(d_D, r, V_padded);
#ifdef PROFILING
        cudaEventRecord(e_p3_stop, 0);
