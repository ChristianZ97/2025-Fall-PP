...
#define BLOCKING_FACTOR 32  // Matches v2 (64x64 data block)
...
// #pragma unroll
    ...
    for (int k = 0; k < BLOCKING_FACTOR; ++k) {
        ...
        d_D[b_start + ty * V_padded + tx] = min(d_D[b_start + ty * V_padded + tx], d_D[b_start + ty * V_padded + k] + d_D[b_start + k * V_padded + tx]);
        // sm[ty][tx] = min(sm[ty][tx], pivot_row_val1 + pivot_col_val1);
        d_D[b_start + ty * V_padded + (tx + HALF_BLOCK)] = min(d_D[b_start + ty * V_padded + (tx + HALF_BLOCK)], d_D[b_start + ty * V_padded + k] + d_D[b_start + k * V_padded + (tx + HALF_BLOCK)]);
        // sm[ty][tx + HALF_BLOCK] = min(sm[ty][tx + HALF_BLOCK], pivot_row_val1 + pivot_col_val2);
        d_D[b_start + (ty + HALF_BLOCK) * V_padded + tx] = min(d_D[b_start + (ty + HALF_BLOCK) * V_padded + tx], d_D[b_start + (ty + HALF_BLOCK) * V_padded + k] + d_D[b_start + k * V_padded + tx]);
        // sm[ty + HALF_BLOCK][tx] = min(sm[ty + HALF_BLOCK][tx], pivot_row_val2 + pivot_col_val1);
        d_D[b_start + (ty + HALF_BLOCK) * V_padded + (tx + HALF_BLOCK)] = min(d_D[b_start + (ty + HALF_BLOCK) * V_padded + (tx + HALF_BLOCK)], d_D[b_start + (ty + HALF_BLOCK) * V_padded + k] + d_D[b_start + k * V_padded + (tx + HALF_BLOCK)]);
...
// #pragma unroll
    ...
    for (int k = 0; k < BLOCKING_FACTOR; ++k) {
        ...
        d_D[self_start + ty * V_padded + tx] = min(d_D[self_start + ty * V_padded + tx], d_D[pivot_start + ty * V_padded + k] + d_D[self_start + k * V_padded + tx]);
        // sm_self[ty][tx] = min(sm_self[ty][tx], pivot_val1 + self_val1);
        d_D[self_start + ty * V_padded + (tx + HALF_BLOCK)] = min(d_D[self_start + ty * V_padded + (tx + HALF_BLOCK)], d_D[pivot_start + ty * V_padded + k] + d_D[self_start + k * V_padded + (tx + HALF_BLOCK)]);
        // sm_self[ty][tx + HALF_BLOCK] = min(sm_self[ty][tx + HALF_BLOCK], pivot_val1 + self_val2);
        d_D[self_start + (ty + HALF_BLOCK) * V_padded + tx] = min(d_D[self_start + (ty + HALF_BLOCK) * V_padded + tx], d_D[pivot_start + (ty + HALF_BLOCK) * V_padded + k] + d_D[self_start + k * V_padded + tx]);
        // sm_self[ty + HALF_BLOCK][tx] = min(sm_self[ty + HALF_BLOCK][tx], pivot_val2 + self_val1);
        d_D[self_start + (ty + HALF_BLOCK) * V_padded + (tx + HALF_BLOCK)] = min(d_D[self_start + (ty + HALF_BLOCK) * V_padded + (tx + HALF_BLOCK)], d_D[pivot_start + (ty + HALF_BLOCK) * V_padded + k] + d_D[self_start + k * V_padded + (tx + HALF_BLOCK)]);
...
// #pragma unroll
    ...
    for (int k = 0; k < BLOCKING_FACTOR; ++k) {
        ...
        d_D[self_start + ty * V_padded + tx] = min(d_D[self_start + ty * V_padded + tx], d_D[self_start + ty * V_padded + k] + d_D[pivot_start + k * V_padded + tx]);
        // sm_self[ty][tx] = min(sm_self[ty][tx], self_val1 + pivot_val1);
        d_D[self_start + ty * V_padded + (tx + HALF_BLOCK)] = min(d_D[self_start + ty * V_padded + (tx + HALF_BLOCK)], d_D[self_start + ty * V_padded + k] + d_D[pivot_start + k * V_padded + (tx + HALF_BLOCK)]);
        // sm_self[ty][tx + HALF_BLOCK] = min(sm_self[ty][tx + HALF_BLOCK], self_val1 + pivot_val2);
        d_D[self_start + (ty + HALF_BLOCK) * V_padded + tx] = min(d_D[self_start + (ty + HALF_BLOCK) * V_padded + tx], d_D[self_start + (ty + HALF_BLOCK) * V_padded + k] + d_D[pivot_start + k * V_padded + tx]);
        // sm_self[ty + HALF_BLOCK][tx] = min(sm_self[ty + HALF_BLOCK][tx], self_val2 + pivot_val1);
        d_D[self_start + (ty + HALF_BLOCK) * V_padded + (tx + HALF_BLOCK)] = min(d_D[self_start + (ty + HALF_BLOCK) * V_padded + (tx + HALF_BLOCK)], d_D[self_start + (ty + HALF_BLOCK) * V_padded + k] + d_D[pivot_start + k * V_padded + (tx + HALF_BLOCK)]);
...
// #pragma unroll
    ...
    for (int k = 0; k < BLOCKING_FACTOR; ++k) {
        ...
        d_D[self_start + ty * V_padded + tx] = min(d_D[self_start + ty * V_padded + tx], d_D[row_start + ty * V_padded + k] + d_D[col_start + k * V_padded + tx]);        
        // sm_self[ty][tx] = min(sm_self[ty][tx], sm_row[ty][k] + sm_col[k][tx]);
        d_D[self_start + ty * V_padded + (tx + HALF_BLOCK)] = min(d_D[self_start + ty * V_padded + (tx + HALF_BLOCK)], d_D[row_start + ty * V_padded + k] + d_D[col_start + k * V_padded + (tx + HALF_BLOCK)]);
        // sm_self[ty][tx + HALF_BLOCK] = min(sm_self[ty][tx + HALF_BLOCK], sm_row[ty][k] + sm_col[k][tx + HALF_BLOCK]);
        d_D[self_start + (ty + HALF_BLOCK) * V_padded + tx] = min(d_D[self_start + (ty + HALF_BLOCK) * V_padded + tx], d_D[row_start + (ty + HALF_BLOCK) * V_padded + k] + d_D[col_start + k * V_padded + tx]);
        // sm_self[ty + HALF_BLOCK][tx] = min(sm_self[ty + HALF_BLOCK][tx], sm_row[ty + HALF_BLOCK][k] + sm_col[k][tx]);
        d_D[self_start + (ty + HALF_BLOCK) * V_padded + (tx + HALF_BLOCK)] = min(d_D[self_start + (ty + HALF_BLOCK) * V_padded + (tx + HALF_BLOCK)], d_D[row_start + (ty + HALF_BLOCK) * V_padded + k] + d_D[col_start + k * V_padded + (tx + HALF_BLOCK)]);
