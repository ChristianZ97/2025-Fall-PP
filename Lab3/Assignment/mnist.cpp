...
#pragma acc data copyin(A[0 : n * k], B[0 : k * m], C[0 : m * 1]) copyout(D[0 : n * m])
 ...
 */
void my_nn(
    float *training_images_flat, int num_images, float *layer1_matrix, float *layer1_bias, float *layer2_matrix, float *layer2_bias, int *result) {
    ...
    LinearLayer(training_images_flat, layer1_matrix, layer1_bias, layer1_output, num_images, LAYER0, LAYER1);
    Sigmoid(layer1_output, num_images, LAYER1);

    // Layer2: Linear layer + Argmax
    LinearLayer(layer1_output, layer2_matrix, layer2_bias, layer2_output, num_images, LAYER1, LAYER2);
...
int main(int argc, char *argv[]) {
    ...
    */
    ...
    my_nn(training_images_flat, num_train_images, layer1_matrix, layer1_bias, layer2_matrix, layer2_bias, result);
    ...
    std::cout << "\n-----     STATS     -----\n";
    std::cout << "Time for initializing CUDA device:     "
              << std::chrono::duration_cast<std::chrono::milliseconds>(read_start - initcuda_start).count() << " m.s.\n";
    std::cout << "Time for reading MNIST data & weights: " << std::chrono::duration_cast<std::chrono::milliseconds>(read_end - read_start).count()
              << " m.s.\n";
    std::cout << "Time for inferencing                 : " << std::chrono::duration_cast<std::chrono::milliseconds>(inference_end - read_end).count()
              << " m.s.\n";
    std::cout << "Time for calculating accuracy        : " << std::chrono::duration_cast<std::chrono::milliseconds>(acc_end - inference_end).count()
              << " m.s.\n";
