...
// PP-HW-UCX
#define UCS_CONFIG_PRINT_TLS UCS_BIT(8)
...
ucp_worker_print_used_tls(ucp_worker_h worker, ucp_worker_cfg_index_t cfg_index)
{
    ...
    // PP-HW-UCX
    ucp_config_print((const ucp_config_t*)&context->config, stdout, NULL, (ucs_config_print_flags_t)UCS_CONFIG_PRINT_TLS);
    ...
    // PP-HW-UCX
    printf("%s\n", ucs_string_buffer_cstr(&strb)); // ucs_info("%s", ucs_string_buffer_cstr(&strb));
}
